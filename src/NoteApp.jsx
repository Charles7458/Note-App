...
function NoteApp() {
    ...
    function downloadNotes() {
        let downloadNotes = []
        if(notes[notes.length-1].title == "Intro" && notes[notes.length-1].content == initialNotes[0].content){
            downloadNotes = notes.slice(0,notes.length-1)
        }
        else{
            downloadNotes = notes
        }
        const json = JSON.stringify(downloadNotes, null, 2);
